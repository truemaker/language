    ...
    def subbed_by(self, other):
        if isinstance(other, Number):
            ...
            try:
                ...
                return None, RTError(other.pos_start, other.pos_end, "Element %s does not exist" % other.value,
                                     self.context)
        else:
            return None, Value.illegal_operation(self, other)
...
class Interpreter:
    ...
    def visit_VarAccessNode(self, node, context):
        ...
        if not value: return res.failure(
            RTError(node.pos_start, node.pos_end, f"Variable '{var_name}' is not defined.", context))
